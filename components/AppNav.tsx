...
export function AppNav() {
  ...
  return (
    <nav className="sticky bottom-0 z-20 px-3 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
      <div className="grid grid-cols-4 items-center rounded-[1.4rem] border border-line/80 bg-paper/95 p-1.5 shadow-[0_-8px_30px_rgba(78,47,21,0.1)] backdrop-blur">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={`relative flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-2xl text-center text-[11px] font-bold transition ${
          ...
          >
            ...
            {item.href === "/session" && live ? (
              <span className="absolute top-1.5 right-3 h-2 w-2 rounded-full bg-sun ring-2 ring-ink" />
            ) : null}
          </Link>
        ))}
      </div>
